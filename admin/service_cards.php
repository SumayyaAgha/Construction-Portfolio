...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($hasEmpty) {
        ...
        for ($i = 0; $i < count($ids); $i++) {
            ...
            $stmt = mysqli_prepare($conn, "UPDATE service_cards SET icon = ?, title = ?, description = ? WHERE id = ?");
            mysqli_stmt_bind_param($stmt, "sssi", $icon, $title, $description, $id);
            mysqli_stmt_execute($stmt);
        }
        $success = "Service cards updated successfully!";
    }
}

$cards_result = mysqli_query($conn, "SELECT * FROM service_cards ORDER BY display_order ASC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Manage Service Cards</title>
...
<body>
    ...
    <div class="main-content">
        <div class="topbar">
            <div>
                <h1>Service Cards</h1>
                <div class="topbar-subtitle">Edit the 6 service cards</div>
            </div>
