...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($labels as $i => $val) {
        if (trim($val) === '' || trim($icons[$i]) === '' || trim($number_values[$i]) === '') {
            $hasEmpty = true;
        }
    }
    ...
    if ($hasEmpty) {
        ...
        for ($i = 0; $i < count($ids); $i++) {
            ...
            $stmt = mysqli_prepare($conn, "UPDATE track_stats SET icon = ?, number_value = ?, label = ? WHERE id = ?");
            mysqli_stmt_bind_param($stmt, "sssi", $icon, $number_value, $label, $id);
            mysqli_stmt_execute($stmt);
        }
        $success = "Track Stats updated successfully!";
    }
}

$stats_result = mysqli_query($conn, "SELECT * FROM track_stats ORDER BY display_order ASC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Manage Track Stats</title>
...
<body>
    ...
    <div class="main-content">
        <div class="topbar">
            <div>
                <h1>Track Stats</h1>
                <div class="topbar-subtitle">Edit the 4 stat cards</div>
            </div>
